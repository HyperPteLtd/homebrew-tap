...
class DmgVerifierTest < Minitest::Test
  class FakeRunner
    ...
    def call(*command)
      ...
      return result("", @failures.fetch(key), success: false) if @failures.key?(key)
    ...
    def result(stdout = "", stderr = "", success: true)
      HyperVpnDmgVerification::CommandRunner::Result.new(stdout: stdout, stderr: stderr, success?: success)
    end
  end
  ...
  def test_rejects_checksum_mismatch_before_mounting
    ...
    error = assert_raises(HyperVpnDmgVerification::VerificationError) do
      HyperVpnDmgVerification::DmgVerifier.new(
        dmg:     @dmg,
        version: "1.0.0",
        sha256:  "0" * 64,
        runner:  runner,

...
class DmgVerifier
  ...
  def verify_app!(app)
    ...
    raise VerificationError, "main executable does not contain arm64: #{architectures.join(' ')}" unless architectures.include?("arm64")
  ...
  def run!(*command)
    ...
    raise VerificationError, "#{command.first} failed#{detail.empty? ? '' : ": #{detail}"}"
  end
end
...
if $PROGRAM_NAME == __FILE__
  ...
  begin
    parser.parse!
    missing = %i[dmg version sha256].reject { |key| options[key] }
    raise OptionParser::MissingArgument, missing.join(", ") unless missing.empty?

    DmgVerifier.new(**options).verify!
    puts "DMG verification passed for Hyper VPN #{options[:version]}"
  rescue OptionParser::ParseError, VerificationError => e
